 ...
 * Author: bmmalone
 *
 * Created on April 15, 2013, 12:02 PM
 */
...
class DFSNode {
public:
    DFSNode() : g(0), h(0), hExact(std::numeric_limits<float>::max()), subnetwork(varset(0)), leaf(0), lastIterationSeen(-1) {
    }

    DFSNode(float g, float h, varset &subnetwork, byte &leaf) :
    g(g), h(h), hExact(std::numeric_limits<float>::max()), subnetwork(subnetwork), leaf(leaf), lastIterationSeen(-1) {
    }
