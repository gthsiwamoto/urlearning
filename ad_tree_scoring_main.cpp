...
int main(int argc, char** argv) {
    ...
    desc.add_options()
            ...
            ("doNotPrune,o", "Add this flag if the scores should NOT be pruned.")
    ...
    if (sf.compare("bic") == 0) {
        scoringFunction = new scoring::BICScoringFunction(network, recordFile, llc, constraints);
    } else if (sf.compare("fnml") == 0) {
        scoringFunction = new scoring::fNMLScoringFunction(network, llc, constraints, regret);
    } else if (sf.compare("bdeu") == 0) {
        scoringFunction = new scoring::BDeuScoringFunction(equivalentSampleSize, network, adTree, constraints);
    }
